...
static int utf8decode(const char *s_in, long *u, int *err)
{
	...
	static const uint32_t overlong[] = { 0x0, 0x80, 0x0800, 0x10000 };
...
Drw *drw_create(Display *dpy, int screen, Window root, uint32_t w, uint32_t h)
{
...
{
	int ty, ellipsis_x = 0;
	uint32_t tmpw, ew, ellipsis_w = 0, ellipsis_len, hash, h0, h1;
	XftDraw *d = NULL;
	...
	static uint32_t nomatches[128], ellipsis_width, invalid_width;
...
uint32_t drw_fontset_getwidth_clamp(Drw *drw, const char *text, uint32_t n)
{
	uint32_t tmp = 0;
